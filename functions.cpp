...
*/
        ...
        if(((millis()-timeMasterControl)>=periodTime) && (inspirationTimeTop == true)){
            ...
            ui->label_flowAlarm->setText(QString::number(maxFlow*0.06,'f',1)); //Conversión de ml/s a l/min
            ui->label_flowAlarm_2->setText(QString::number(maxFlow*0.06,'f',1)); //Conversión de ml/s a l/min "0002"

            if(((cicleCounter-alarmCurrenCiclosVol)>=4) && ((cicleCounter-alarmCurrenCiclosFR)>=6)){

            if((maxFlow*0.06 >= flowLm+flowLm*(flowError/100))  && (flagAlarmVolMinMax  == false)){
                flagAlarmVolMinMax = true;
                alarmOn = true;
                alarmText+= " Vol/min alto.|";
            }
            if((maxFlow*0.06 <= flowLm-flowLm*(flowError/100))  && (flagAlarmVolMinMin   == false)){
            ...
            if((cicleCounter-alarmCurrenCiclosFR)>=4 && (cicleCounter-alarmCurrenCiclosVol)>=4){
                if((frCurrent >= FRv+FRv*(frError/100)) && (flagAlarmFRMax == false)){
                    frCounter++;
                    if(frCounter>=2){
                        flagAlarmFRMax = true;
                        alarmOn = true;
                ...
                if((frCurrent <= FRv-FRv*(frError/100)) && (flagAlarmFRMin == false)){
                    frCounter++;
                    if(frCounter>=2){
                        flagAlarmFRMin = true;
                        alarmOn = true;
